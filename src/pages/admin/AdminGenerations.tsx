...
export default function AdminGenerations() {
  ...
  const list = useQuery({
    queryKey: ["admin", "generations", { page, status }],
    queryFn: () =>
      generationsApi.listAdmin({
        page,
        pageSize: 20,
  ...
  return (
    <>
      <PageHeader title="Generations" description="Every resume generated by your bidders." />
                ...
                : list.data?.data.map((g) => (
                    <tr key={g.id} className="hover:bg-slate-50 dark:hover:bg-slate-900/40">
                      ...
                      <Td className="text-right">
                        <Link to={`/admin/generations/${g.id}`} className="btn-ghost">
                          View
                        </Link>
                      </Td>
                    </tr>
